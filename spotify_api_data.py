...
client_id = '################################' #Replace this with your client ID from Spotify Dev.
client_secret = '############################' #Replace this with your client secret from Spotify Dev.
...
def genre_build_data():
    '''Builds the specified n-genre dataset in the new format, without saving large volume of features. '''

    #Initialize the Spotify client and hard code the genres that we want to highlight.
    #Here, we choose pop country, outlaw, and bluegrass/folk style as things to differentiate.
    spot = spotify_login(client_id, client_secret)
    ...
    for i in range(len(playlists)):
        ...
        labels = np.full(len(urllist), i) #We're specifying each playlist as a label/class.
...
def download_data():
    '''Loads in the saved URLs from csv that we pulled from API and downloads the 30s sample data.'''
    ...
    return directory #RETURNS THE SAVED PATH
